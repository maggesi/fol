...
simplify E E.
/*
example (add (mul (add (mul (const 0) (var "x")) (const 1)) (const 3)) (const 12)).

main X :- example A, simplify A X.
*/
test 1 "first simplify with cut" A X:- A = (add (mul (add (mul (const 0) (var "x")) (const 1)) (const 3)) (const 12)), simplify A X.
